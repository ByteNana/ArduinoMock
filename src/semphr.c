...
SemaphoreHandle_t xSemaphoreCreateBinary(void) { return (SemaphoreHandle_t)sem_alloc(0, 1); }

SemaphoreHandle_t xSemaphoreCreateMutex(void) { return (SemaphoreHandle_t)sem_alloc(1, 1); }
...
BaseType_t xSemaphoreTake(SemaphoreHandle_t sem, TickType_t wait) {
    ...
    while (s->count == 0) {
        ...
        int rc = (wait == portMAX_DELAY) ? pthread_cond_wait(&s->cv, &s->mtx)
